...
function Viewform({ records, onDeleteRecord, onEditRecord, onExportClick }) {
  ...
  // Show only 20 items (2 pages) by default. If searching or filtering, show all matching items.
  const baseRecords = (searchTerm.trim() || filterValue !== 'All')
    ? groupedRecords
    : groupedRecords.slice(0, 20);

  const totalPages = Math.ceil(baseRecords.length / itemsPerPage);
